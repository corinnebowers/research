...
https://hub.arcgis.com/datasets/355b1002325d47e580d57c478778c97f_0
https://gis-sonomacounty.hub.arcgis.com/datasets/parcels-public-shapefile (newer)
```{r}
## older version
# parcels_old <-
...
```{r}
## update with Sonoma County GIS building + parcel information
load('C:/Users/cbowers/OneDrive/classes/old/GEOLSCI240 data science for geoscience/geolsci240/hw5_checkpoint.Rdata')
nonzero.buffer <- nonzero.buffer %>% 
  rasterToPolygons %>% 
  st_as_sf %>%
  st_union %>% 
  st_transform(st_crs(sonoma))
crs_ca2 <- '+proj=lcc +lat_1=38.33333333333334 +lat_2=39.83333333333334 +lat_0=37.66666666666666 +lon_0=-122 +x_0=2000000 +y_0=500000.0000000002 +datum=NAD83 +units=us-ft +no_defs' 
bldg_buffer <- bldg_sonoma[nonzero.buffer,] %>% 
  st_transform(crs_ca2) %>% mutate(bldg_sqft = st_area(.)) %>% 
  st_transform(st_crs(sonoma)) %>% st_centroid
# parcels_buffer <- parcels_new[nonzero.buffer,] %>% st_buffer(0)
...
## summarize NSI foundation info by CBG
nsi1.found <- nsi1 %>% 
...
## save out
save(res.buildings, file = 'C:/Users/cbowers/Desktop/buildings.Rdata')
...
res.buildings %>%
  st_drop_geometry %>%
  group_by(GEOID) %>%
  summarize(median_value = median(V601Stru, na.rm = TRUE), .groups = 'drop') %>%
...
```{r}
ggplot(buildings %>% mutate(id = 1:nrow(.))) + 
  geom_sf(aes(color = id==11654))
